...
proc registerVirtual_getBpm*[T: AudioStream](Self: typedesc[T]) =
  Self.vmethods[newStringName"_get_bpm"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
...
proc registerVirtual_hasLoop*[T: AudioStream](Self: typedesc[T]) =
  Self.vmethods[newStringName"_has_loop"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
