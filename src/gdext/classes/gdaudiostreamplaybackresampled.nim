...
proc registerVirtual_mixResampled*[T: AudioStreamPlaybackResampled](Self: typedesc[T]) =
  Self.vmethods[stringName"_mix_resampled"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
...
proc registerVirtual_getStreamSamplingRate*[T: AudioStreamPlaybackResampled](Self: typedesc[T]) =
  Self.vmethods[stringName"_get_stream_sampling_rate"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
