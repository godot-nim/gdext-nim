...
proc registerVirtual_setUid*[T: ResourceFormatSaver](Self: typedesc[T]) =
  Self.vmethods[newStringName"_set_uid"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
...
proc registerVirtual_recognize*[T: ResourceFormatSaver](Self: typedesc[T]) =
  Self.vmethods[newStringName"_recognize"] = proc (p_instance: ClassInstancePtr; p_args: ptr UncheckedArray[ConstTypePtr]; r_ret: TypePtr) {.gdcall.} =
