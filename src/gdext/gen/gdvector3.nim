...
# `in(Vector3 Dictionary)`
# `in(Vector3 Array)`
# `in(Vector3 PackedVector3Array)`
func `*`*(left: Vector3; right: Quaternion): Vector3 = {.noSideEffect.}: `*(Vector3 Quaternion)`(getPtr left, getPtr right, addr result)
func `*`*(left: Vector3; right: Basis): Vector3 = {.noSideEffect.}: `*(Vector3 Basis)`(getPtr left, getPtr right, addr result)
func `*`*(left: Vector3; right: Transform3D): Vector3 = {.noSideEffect.}: `*(Vector3 Transform3D)`(getPtr left, getPtr right, addr result)
proc load_Vector3_operators {.execon: staticevents.init_engine.on_load_builtinclassOperator.} =
  `*(Vector3 Quaternion)` = load(opMultiply, VariantType_Vector3, VariantType_Quaternion)
  `*(Vector3 Basis)` = load(opMultiply, VariantType_Vector3, VariantType_Basis)
  `*(Vector3 Transform3D)` = load(opMultiply, VariantType_Vector3, VariantType_Transform3D)
